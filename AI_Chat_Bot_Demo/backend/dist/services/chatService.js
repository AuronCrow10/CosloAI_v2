...
exports.ChatServiceError = void 0;
exports.generateBotReplyForSlug = generateBotReplyForSlug;
const config_1 = require("../bots/config");
const client_1 = require("../knowledge/client");
...
exports.ChatServiceError = ChatServiceError;
    ...
    function: {
        ...
        parameters: {
            ...
            properties: {
                ...
                datetime: {
                    type: "string",
                    description: "Requested appointment date and time in ISO 8601 (e.g. 2025-11-23T15:00:00). Treat as local time in the business's time zone."
                }
            },
...
function getBookingInstructions() {
    return ("You can create appointments by calling the tool `book_appointment`.\n" +
 ...
 */
async function generateBotReplyForSlug(slug, rawMessage, options = {}) {
    ...
    if (!botConfig.knowledgeClientId) {
        throw new ChatServiceError("This bot has no knowledge base configured yet. Ask the owner to set up content & crawl the site.", 400);
    }
    ...
    const results = await (0, client_1.searchKnowledge)({
    ...
    const messages = [
        ...
        {
            role: "system",
            content: "You are given CONTEXT extracted from the business website.\n" +
    ...
    if (options.conversationId) {
        const historyMessages = await (0, conversationService_1.getConversationHistoryAsChatMessages)(options.conversationId);
        if (historyMessages.length > 0) {
            messages.push({
                role: "system",
                content: "Below is the recent conversation history with this user. Use it to understand context, references and follow-ups."
            });
    ...
    if (!bookingEnabled) {
        return await (0, client_2.getChatCompletion)({ messages, maxTokens: 400 });
    }
    // 5) Booking-enabled path: use tools
    const firstResponse = await client_2.openai.chat.completions.create({
        model: "gpt-4.1-mini",
        messages,
        max_tokens: 400,
        tools: [bookingTool],
    ...
    try {
        ...
        const bookingResult = {
            success: false,
            errorMessage: "Invalid booking data. Please provide your name, phone, service and desired date/time clearly."
        };
        ...
        const secondResponse = await client_2.openai.chat.completions.create({
            model: "gpt-4.1-mini",
    ...
    const bookingResult = await (0, bookingService_1.handleBookAppointment)(slug, args);
