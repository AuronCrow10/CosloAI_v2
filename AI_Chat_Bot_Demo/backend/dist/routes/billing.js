...
router.post("/bots/:id/pricing-preview", async (req, res) => {
    try {
        ...
        const bot = await prisma_1.prisma.bot.findUnique({ where: { id: botId } });
        if (!bot) {
            return res.status(404).json({ error: "Bot not found" });
        }
        const body = (req.body || {});
        const flags = {
            useDomainCrawler: typeof body.useDomainCrawler === "boolean"
                ? body.useDomainCrawler
                : bot.useDomainCrawler,
            usePdfCrawler: typeof body.usePdfCrawler === "boolean"
                ? body.usePdfCrawler
                : bot.usePdfCrawler,
            channelWeb: typeof body.channelWeb === "boolean"
                ? body.channelWeb
                : bot.channelWeb,
            channelWhatsapp: typeof body.channelWhatsapp === "boolean"
                ? body.channelWhatsapp
                : bot.channelWhatsapp,
            channelMessenger: typeof body.channelMessenger === "boolean"
                ? body.channelMessenger
                : bot.channelMessenger,
            channelInstagram: typeof body.channelInstagram === "boolean"
                ? body.channelInstagram
                : bot.channelInstagram,
            useCalendar: typeof body.useCalendar === "boolean"
                ? body.useCalendar
                : bot.useCalendar
        };
        const pricing = await (0, billingService_1.computeBotPricingForBot)(flags);
        return res.json({
...
router.post("/bots/:id/checkout", async (req, res) => {
    try {
        ...
        const pricing = await (0, billingService_1.computeBotPricingForBot)({
        ...
        if (!stripeCustomerId) {
            const customer = await billingService_1.stripe.customers.create({
        ...
        const session = await billingService_1.stripe.checkout.sessions.create({
            mode: "subscription",
...
router.post("/bots/:id/cancel-subscription", async (req, res) => {
    try {
        ...
        if (billingService_1.stripe) {
            try {
                await billingService_1.stripe.subscriptions.cancel(stripeSubscriptionId);
            }
            catch (err) {
        ...
        const updatedSub = await prisma_1.prisma.subscription.update({
            where: { id: bot.subscription.id },
            data: {
                status: "CANCELED"
            }
        });
        const updatedBot = await prisma_1.prisma.bot.update({
            where: { id: botId },
            data: {
                status: "CANCELED"
            }
